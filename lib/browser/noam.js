    ...
    var tree = (function() {
      ...
      /*
        ...
        letters represent any regular expression):
          ...
          # $+aa* => a* <- added by @oruji
          # a+bb*a => ($+bb*)a <- added by @oruji
          # a+abb* => a($+bb*) <- added by @oruji
          # aaa*+a => aa* <- added by @oruji
          # a(b+c) => ab+ac <- added by @oruji
          # a*(ba*)* => (a+b)* <- added by @oruji
          # (a*b)*a* => (a+b)* <- added by @oruji
          # a*a to aa* <- added by @oruji
          # aa* to a*a <- added by @oruji
          # a*+a*bb*a* => a*($+bb*)a* <- added by @oruji
      ...
      */
      ...
      function _regex_simplify_26(tree, fsmCache) {
	    ...
	    // $+aa* => a* <- added by @oruji
      function _regex_simplify_99(tree, fsmCache) {
        if (tree.tag === tags.ALT && tree.choices.length >= 2) {
          var epsIndex = -1;
          var kstarIndex = -1;
          var sameStar = -1;
          ...
          for (var i=0; i < tree.choices.length; i++) {
            ...
            } else if (tree.choices[i].tag === tags.SEQ && tree.choices[i].elements.length === 2) {
              elemIndex = i;

              var starLit = null;
              var normLit = null;
              for (var j=0; j<tree.choices[i].elements.length;j++) {
                if (tree.choices[i].elements[j].tag === tags.KSTAR) {
                  kstarIndex = j;
                  starLit = tree.choices[i].elements[j].expr.obj;

                } else if (tree.choices[i].elements[j].tag === tags.LIT) {
                  sameStar = j;
                  normLit = tree.choices[i].elements[j].obj;
                }
              }
            }
            if (epsIndex >= 0 && kstarIndex >= 0 && sameStar >= 0 && elemIndex >= 0) {
              if (starLit === normLit) {
                tree.choices[elemIndex].elements.splice(sameStar, 1)
                tree.choices.splice(epsIndex, 1);
      ...
      // a+bb*a => ($+bb*)a <- added by @oruji
      function _regex_simplify_98(tree, fsmCache) {
        if (tree.tag === tags.ALT && tree.choices.length >=2) {
          for (var i=0; i<tree.choices.length-1; i++) {
            // first choice is not sequence "a"+bb*a
            if (tree.choices[i].tag !== tags.SEQ) {
              // i = firstChoice, j = secondChoice
              for (var j=i+1; j<tree.choices.length; j++) {
                if (tree.choices[j].tag === tags.SEQ && tree.choices[j].elements.length == 3) {
                  // if first choice ("a"+bb*a) and last of second (a+bb*"a") are equal
                  if (noam.util.areEquivalent(getLast(tree.choices[j].elements), tree.choices[i])) {
                    var first = null;
                    var last = null;
                    var hasStar = false;
                    
                    if (tree.choices[j].elements[0].tag === tags.KSTAR) {
                      first = tree.choices[j].elements[0].expr;
                      last = tree.choices[j].elements[1];
                      hasStar = true;
                    }
                    
                    else if (tree.choices[j].elements[1].tag === tags.KSTAR) {
                      last = tree.choices[j].elements[1].expr;
                      first = tree.choices[j].elements[0]
                      hasStar = true;
                    }
                    
                    if (noam.util.areEquivalent(first, last) && hasStar) {
                      // last common element "b" of first choice
                      var last = tree.choices[i];
                      // first choice will be lambda
                      var rest1 = makeEps();
                      // select elements of second choice except last element
                      var rest2 = makeSeq(tree.choices[j].elements.slice(0, tree.choices[j].elements.length-1));

                      // $+c | make a choice by $ & c
                      var _alt = makeAlt([rest1, rest2]);
                      // ($+c)b | sequence $+c made with last common element
                      var _seq = makeSeq([_alt, last]);

                      // replace first choice "ab" by created "(a+c)b"
                      tree.choices[i] = _seq;
                      // remove second choice "cb"
                      tree.choices.splice(j, 1);
            ...
            // first choice is sequence bb*a+a
            } else if (tree.choices[i].tag === tags.SEQ && tree.choices[i].elements.length == 3) {
              // i = firstChoice, j = secondChoice
              for (var j=i+1; j<tree.choices.length; j++) {
                if (tree.choices[j].tag !== tags.SEQ) {
                  // if last choice (bb*a+"a") and last first choice (bb*"a"+a) are equal
                  if (noam.util.areEquivalent(getLast(tree.choices[i].elements), tree.choices[j])) {
                    var first = null;
                    var last = null;
                    var hasStar = false;
                    
                    if (tree.choices[i].elements[0].tag === tags.KSTAR) {
                      first = tree.choices[i].elements[0].expr;
                      last = tree.choices[i].elements[1];
                      hasStar = true;
                      
                    } else if (tree.choices[i].elements[1].tag === tags.KSTAR) {
                      last = tree.choices[i].elements[1].expr;
                      first = tree.choices[i].elements[0]
                      hasStar = true;
                    }

                    if (noam.util.areEquivalent(first, last) && hasStar) {
                      // last common element "b" of first choice
                      var last = tree.choices[j];
                      // first choice will be lambda
                      var rest1 = makeSeq(tree.choices[i].elements.slice(0, tree.choices[i].elements.length-1));
                      // select elements of second choice except last element
                      var rest2 = makeEps();
                      ...
                      // replace first choice "cb" by created "(c+$)b"
                      tree.choices[i] = _seq;
                      // remove second choice "b"
                      tree.choices.splice(j, 1);
      ...
      // a+abb* => a($+bb*) <- added by @oruji
      function _regex_simplify_97(tree, fsmCache) {
        if (tree.tag === tags.ALT && tree.choices.length >= 2) {
          // loop over first choice
          for (var i=0; i<tree.choices.length-1; i++) {
            // first choice is not sequence a+abb*
            if (tree.choices[i].tag !== tags.SEQ) {
              // i = firstChoice, j = secondChoice
              for (var j=i+1; j<tree.choices.length; j++) {
                if (tree.choices[j].tag === tags.SEQ && tree.choices[j].elements.length == 3) {
                  // if first choice ("a"+abb*) and first of second (a+"a"bb*) are equal
                  if (noam.util.areEquivalent(tree.choices[i], tree.choices[j].elements[0])) {
                    var first = null;
                    var last = null;
                    var hasStar = false;
                    
                    if (getLast(tree.choices[j].elements).tag === tags.KSTAR) {
                      first = getLast(tree.choices[j].elements).expr;
                      last = getLast(tree.choices[j].elements, 2);
                      hasStar = true;
                      
                    } else if (getLast(tree.choices[j].elements, 2).tag === tags.KSTAR) {
                      last = getLast(tree.choices[j].elements, 2).expr;
                      first = getLast(tree.choices[j].elements);
                      hasStar = true;
                    }

                    if (noam.util.areEquivalent(first, last) && hasStar) {
                      // first common element "a" of first choice
                      var first = tree.choices[i];
                      // first choice will be "$"
                      var rest1 = makeEps();
                      // select elements of second choice except first element "b"
                      var rest2 = makeSeq(tree.choices[j].elements.slice(1, tree.choices[j].elements.length));

                      // $+b | make a choice by $ & c
                      var _alt = makeAlt([rest1, rest2]);
                      // a($+b) | sequence $+b made with first common element
                      var _seq = makeSeq([first, _alt]);

                      // replace first choice "a" by created "a($+b)"
                      tree.choices[i] = _seq;
                      // remove second choice "ab"
                      tree.choices.splice(j, 1);
            ...
            // first choice is sequence -> abb*+a
            } else if (tree.choices[i].tag === tags.SEQ && tree.choices[i].elements.length == 3) {
              // i = firstChoice, j = secondChoice
              for (var j=i+1; j<tree.choices.length; j++) {
                if (tree.choices[j].tag !== tags.SEQ) {
                  // if first of first choice ("a"bb*+a) and second choice (abb*+"a") are equal
                  if (noam.util.areEquivalent(tree.choices[i].elements[0], tree.choices[j])) {
                    var first = null;
                    var last = null;
                    var hasStar = false;
                    
                    if (getLast(tree.choices[i].elements).tag === tags.KSTAR) {
                      first = getLast(tree.choices[i].elements).expr;
                      last = getLast(tree.choices[i].elements, 2);
                      hasStar = true;
                      
                    } else if (getLast(tree.choices[i].elements, 2).tag === tags.KSTAR) {
                      last = getLast(tree.choices[i].elements, 2).expr;
                      first = getLast(tree.choices[i].elements);
                      hasStar = true;
                    }

                    if (noam.util.areEquivalent(first, last) && hasStar) {
                      // first common element "a" from second choice
                      var first = tree.choices[j];
            
                      // select elements of first choice except first element
                      var rest1 = makeSeq(tree.choices[i].elements.slice(1, tree.choices[i].elements.length));
            
                      // second choice will be $
                      var rest2 = makeEps();

                      // b+$ | make a choice by b & $
                      var _alt = makeAlt([rest1, rest2]);
                      // a(b+$) | sequence b+$ made with last common element
                      var _seq = makeSeq([first, _alt]);

                      // replace first choice "ab" by created "a(b+$)"
                      tree.choices[i] = _seq;
                      // remove second choice "a"
                      tree.choices.splice(j, 1);
      ...
      // aaa*+a => aa* <- added by @oruji
      function _regex_simplify_96(tree, fsmCache) {
        if (tree.tag === tags.ALT && tree.choices.length >= 2) {
          var litIndex = -1;
          var kstarIndex = -1;
          var sameStar = -1;
          ...
          for (var i=0; i < tree.choices.length; i++) {
            if (tree.choices[i].tag === tags.LIT) {
              litIndex = i;
              ...
              for (var j=0; j<tree.choices[i].elements.length;j++) {
                if (tree.choices[i].elements[j].tag === tags.KSTAR) {
                  kstarIndex = j;
                  starLit[starNum] = tree.choices[i].elements[j].expr.obj;
                  starNum++;

                } else if (tree.choices[i].elements[j].tag === tags.LIT) {
                  sameStar = j;
                  normLit[litNum] = tree.choices[i].elements[j].obj;
                  litNum++;
                }
              }
            }
          ...
          if (litIndex >= 0 && kstarIndex >= 0 && sameStar >= 0 && elemIndex >= 0 && starNum === 1 && litNum === 2) {
            if (starLit[0] === normLit[0] && starLit[0] === normLit[1] && starLit[0] !== null) {
              tree.choices[elemIndex].elements.splice(sameStar, 1)
              tree.choices.splice(litIndex, 1);
              return true;  
            }
          }
        }
      ...
      // a(b+c) => ab+ac <- added by @oruji
      function _regex_simplify_95(tree, fsmCache) {
        if(tree.tag === tags.SEQ) {
          if(tree.elements[tree.elements.length-1].tag === tags.ALT) {
            for (var i = 0; i < tree.elements[tree.elements.length-1].choices.length; i++) {
              var makeSeqEls = [];
              for (var j = 0; j < tree.elements.length-1;j++) {
                makeSeqEls.push(tree.elements[j]);
              }
              makeSeqEls.push(tree.elements[tree.elements.length-1].choices[i]);
              tree.elements[tree.elements.length-1].choices[i] = makeSeq(makeSeqEls);
            }
            tree.elements.splice(0, tree.elements.length-1);
      ...
      // a*(ba*)* => (a+b)* <- added by @oruji
      function _regex_simplify_94(tree, fsmCache) {
        var treeBack = noam.util.clone(tree);
        try {
          if (tree.tag === tags.SEQ && tree.elements.length >= 2) {
            // change a*a to aa*
            var mySimplify = false;
            do {
              mySimplify = _simplify_recursion(tree, _regex_simplify_92);
            } while (mySimplify);
            
            for(var i = 0; i <= tree.elements.length-1;i++) {
              if (tree.elements[i].tag === tags.KSTAR) {
                if (tree.elements[i+1].tag === tags.KSTAR) {
                  if (tree.elements[i+1].expr.tag === tags.SEQ) {
                    if (tree.elements[i+1].expr.elements.length >= 2) {
                      if (noam.util.areEquivalent(tree.elements[i],
                        getLast(tree.elements[i+1].expr.elements))) {
                        tree.elements[i+1].expr.elements.splice(-1,1);
                        
                        var mySeq = makeSeq(tree.elements[i+1].expr.elements);
                        var myAlt = makeAlt([tree.elements[i].expr, mySeq]);
                        
                        tree.elements[i+1].expr = myAlt;
        ...
        // undo aa* to a*a
        copyAndDeleteProperties(tree, treeBack)
        
        return false;
      }

      // (a*b)*a* => (a+b)* <- added by @oruji
      function _regex_simplify_93(tree, fsmCache) {
        var treeBack = noam.util.clone(tree);
        try {
          if (tree.tag === tags.SEQ && tree.elements.length >= 2) {
            // change aa* to a*a
            var mySimplify = false;
            do {
              mySimplify = _simplify_recursion(tree, _regex_simplify_91);
            } while (mySimplify);

            for(var i = 0; i <= tree.elements.length-1;i++) {
              if (tree.elements[i+1].tag === tags.KSTAR) {
                if (tree.elements[i].tag === tags.KSTAR) {
                  if (tree.elements[i].expr.tag === tags.SEQ) {
                    if (tree.elements[i].expr.elements.length >= 2) {
                      if (noam.util.areEquivalent(tree.elements[i+1], tree.elements[i].expr.elements[0])) {
                        tree.elements[i].expr.elements.shift();
                        
                        var mySeq = makeSeq(tree.elements[i].expr.elements);
                        var myAlt = makeAlt([tree.elements[i+1].expr, mySeq]);
                        
                        tree.elements[i].expr = myAlt;
      ...
      // a*a to aa* <- added by @oruji
      function _regex_simplify_92(tree, fsmCache) {
        var hasA = false;
        try {
          if (tree.tag === tags.SEQ && tree.elements.length >= 2) {
            for (var i = 0; i < tree.elements.length - 1; i++) {
              if (tree.elements[i].tag === tags.KSTAR) {
                if (noam.util.areEquivalent(tree.elements[i].expr, tree.elements[i+1])) {
                  tree.elements[i] = tree.elements[i+1];
                  tree.elements[i+1] = makeKStar(tree.elements[i]);
                  hasA = true;
                }
              }
            }
      ...
      // aa* to a*a <- added by @oruji
      function _regex_simplify_91(tree, fsmCache) {
        var hasA = false;
        try {
          if (tree.tag === tags.SEQ && tree.elements.length >= 2) {
            for (var i = tree.elements.length-1; i >= 0; i--) {
              if (tree.elements[i].tag === tags.KSTAR) {
                if (noam.util.areEquivalent(tree.elements[i].expr, tree.elements[i-1])) {
                  tree.elements[i] = tree.elements[i-1];
                  tree.elements[i-1] = makeKStar(tree.elements[i]);
      ...
      // a*+a*bb*a* => a*($+bb*)a* <- added by @oruji
      function _regex_simplify_90(tree, fsmCache) {
        if (tree.tag === tags.ALT && tree.choices.length >=2) {
          for (var i=0; i<tree.choices.length-1; i++) {
            // first choice is KSTAR "a*"+a*bb*a*
            if (tree.choices[i].tag === tags.KSTAR) {
              // i = firstChoice, j = secondChoice
              for (var j=i+1; j<tree.choices.length; j++) {
                if (tree.choices[j].tag === tags.SEQ && tree.choices[j].elements.length === 4) {
                  // if first choice ("a"+bb*a) and last of second (a+bb*"a") are equal
                  if (noam.util.areEquivalent(getLast(tree.choices[j].elements), tree.choices[i])
                      && noam.util.areEquivalent(tree.choices[j].elements[0], tree.choices[i])) {
                    var first = null;
                    var last = null;
                    var hasStar = false;
                    
                    if (tree.choices[j].elements[1].tag === tags.KSTAR) {
                      first = tree.choices[j].elements[1].expr;
                      last = tree.choices[j].elements[2];
                      hasStar = true;
                    }
                    
                    else if (tree.choices[j].elements[2].tag === tags.KSTAR) {
                      last = tree.choices[j].elements[2].expr;
                      first = tree.choices[j].elements[1]
                      hasStar = true;
                    }
                    
                    if (noam.util.areEquivalent(first, last) && hasStar) {
                      // common element "a*" of (a*+a*bb*a*)
                      var last = tree.choices[i];
                      // first choice will be $ ("$"+a*bb*a*)
                      var rest1 = makeEps();
                      // select elements of second choice except last and first (bb*)
                      var rest2 = makeSeq(tree.choices[j].elements.slice(1, tree.choices[j].elements.length-1));

                      // make choice -> $+bb*
                      var _alt = makeAlt([rest1, rest2]);
                      // make sequence -> a*($+bb*)a*
                      var _seq = makeSeq([last, _alt, last]);

                      // replace first choice a* by created a*($+bb*)a*
                      tree.choices[i] = _seq;
                      // remove second choices
                      tree.choices.splice(j, 1);
            ...
            // first choice is sequence a*bb*a*+a*
            } else if (tree.choices[i].tag === tags.SEQ && tree.choices[i].elements.length === 4) {
              // i = firstChoice, j = secondChoice
              for (var j=i+1; j<tree.choices.length; j++) {
                if (tree.choices[j].tag === tags.KSTAR) {
                  // if last choice (bb*a+"a") and last first choice (bb*"a"+a) are equal
                  if (noam.util.areEquivalent(getLast(tree.choices[i].elements), tree.choices[j])
                      && noam.util.areEquivalent(tree.choices[i].elements[0], tree.choices[j])) {
                    var first = null;
                    var last = null;
                    var hasStar = false;
                    
                    if (tree.choices[i].elements[1].tag === tags.KSTAR) {
                      first = tree.choices[i].elements[1].expr;
                      last = tree.choices[i].elements[2];
                      hasStar = true;
                      
                    } else if (tree.choices[i].elements[2].tag === tags.KSTAR) {
                      last = tree.choices[i].elements[2].expr;
                      first = tree.choices[i].elements[1]
                      hasStar = true;
                    }
                    ...
                    if (noam.util.areEquivalent(first, last) && hasStar) {
                      ...
                      var last = tree.choices[j];
                      // select elements of second choice except last element
                      var rest1 = makeSeq(tree.choices[i].elements.slice(1, tree.choices[i].elements.length-1));
                      // second choice will be $
                      var rest2 = makeEps();

                      // make a choice bb*+$
                      var _alt = makeAlt([rest1, rest2]);
                      // make a sequence a*(bb*+$)a*
                      var _seq = makeSeq([last, _alt, last]);
      ...
      _regex_simplification_patterns.push({ 'pattern' : "$+aa* => a* <- added by @oruji", 'type' : 'structure', 'function' : _regex_simplify_99});
      _regex_simplification_patterns.push({ 'pattern' : "a+bb*a => ($+bb*)a <- added by @oruji", 'type' : 'structure', 'function' : _regex_simplify_98});
  	  _regex_simplification_patterns.push({ 'pattern' : "a+abb* => a($+bb*) <- added by @oruji", 'type' : 'structure', 'function' : _regex_simplify_97});
   	  _regex_simplification_patterns.push({ 'pattern' : "aaa*+a => aa* <- added by @oruji", 'type' : 'structure', 'function' : _regex_simplify_96});
      _regex_simplification_patterns.push({ 'pattern' : "a*(ba*)* => (a+b)* <- added by @oruji", 'type' : 'structure', 'function' : _regex_simplify_94});
      _regex_simplification_patterns.push({ 'pattern' : "(a*b)*a* => (a+b)* <- added by @oruji", 'type' : 'structure', 'function' : _regex_simplify_93});
      _regex_simplification_patterns.push({ 'pattern' : "a*+a*bb*a* => a*($+bb*)a* <- added by @oruji", 'type' : 'structure', 'function' : _regex_simplify_90});
