    ...
    var tree = (function() {
      ...
      /*
        ...
        letters represent any regular expression):
          ...
          # (a+$)* => a*
          # (ab+ac) => a(b+c)
          # a+ab => a($+b) <- added by @oruji
          # a*aa* => aa*
          # (ab+cb) => (a+c)b
          # aaa*+a => aa* -> added by @oruji
          # a+ba => ($+b)a <- added by @oruji
          # a*($+b(a+b)*) => (a+b)*
          # ($+(a+b)*a)b* => (a+b)*
          # (a()) => ()
          # ()* => ()
      ...
      */
      ...
      function _regex_simplify_97(tree, fsmCache) {
        if (tree.tag === tags.ALT && tree.choices.length >=2) {
          for (var i=0; i<tree.choices.length-1; i++) {
            ...
            } else if (tree.choices[i].tag === tags.SEQ) {
              for (var j=i+1; j<tree.choices.length; j++) {
                if (tree.choices[j].tag !== tags.SEQ) {
      ...
      _regex_simplification_patterns.push({ 'pattern' : "a+ab => a($+b) -> added by @oruji", 'type' : 'structure', 'function' : _regex_simplify_97});
      ...
      _regex_simplification_patterns.push({ 'pattern' : "a+ba => ($+b)a -> added by @oruji", 'type' : 'structure', 'function' : _regex_simplify_98});
