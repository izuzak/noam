    ...
    var tree = (function() {
      ...
      */
      ...
      function _regex_simplify_98(tree, fsmCache) {
        // if we have more than 1 choices in input regex
        if (tree.tag === tags.ALT && tree.choices.length >=2) {
          // loop over choices
          for (var i=0; i<tree.choices.length-1; i++) {
            // first choice is not sequence a+bb*a
            if (tree.choices[i].tag !== tags.SEQ) {
              // loop over second choice so that i is first choice index and j is second choice at the same time <- added by @oruji
              for (var j=i+1; j<tree.choices.length; j++) {
                if (tree.choices[j].tag === tags.SEQ && tree.choices[j].elements.length == 3) {
					
                  // if literal of first choice "b" and last element of second seq choice are equal <- added by @oruji
                  if (noam.util.areEquivalent(tree.choices[j].elements[tree.choices[j].elements.length-1],
                                              tree.choices[i])) {
                    ...
                    if (noam.util.areEquivalent(first, last) && hasStar) {
                      ...
                      // last common element "b" of first choice  <- added by @oruji
                      var last = tree.choices[i];
                      // first choice will be lambda <- added by @oruji
                      var rest1 = makeEps();
                      // select elements of second choice except last element <- added by @oruji
                      var rest2 = makeSeq(tree.choices[j].elements.slice(0, tree.choices[j].elements.length-1));

                      // $+c | make a choice by $ & c <- added by @oruji
                      var _alt = makeAlt([rest1, rest2]);
                      // ($+c)b | sequence $+c made with last common element <- added by @oruji
                      var _seq = makeSeq([_alt, last]);

                      // replace first choice "ab" by created "(a+c)b" <- added by @oruji
                      tree.choices[i] = _seq;
            ...
            // first choice is sequence bb*a+a
              ...
              for (var j=i+1; j<tree.choices.length; j++) {
                if (tree.choices[j].tag !== tags.SEQ) {
                  ...
                  // if unSEQ of first choice "b" and last element of second seq choice are equal <- added by @oruji
                  if (noam.util.areEquivalent(tree.choices[i].elements[tree.choices[i].elements.length-1],
                                              tree.choices[j])) {
                    ...
                    if (noam.util.areEquivalent(first, last) && hasStar) {
                      ...
                      // replace first choice "cb" by created "(c+$)b" <- added by @oruji
                      tree.choices[i] = _seq;
            ...
            if (tree.choices[i].tag !== tags.SEQ) {
              ...
              for (var j=i+1; j<tree.choices.length; j++) {
                if (tree.choices[j].tag === tags.SEQ && tree.choices[j].elements.length == 3) {
                  ...
                  // if unSEQ of first choice "a" and first element of second seq choice are equal <- added by @oruji
                  if (noam.util.areEquivalent(tree.choices[j].elements[0], tree.choices[i])) {
                    ...
                    if (noam.util.areEquivalent(first, last) && hasStar) {
                      // first common element "a" of first choice  <- added by @oruji
                      var first = tree.choices[i];
                      // first choice will be "$" <- added by @oruji
                      var rest1 = makeEps();
                      // select elements of second choice except first element "b" <- added by @oruji
                      var rest2 = makeSeq(tree.choices[j].elements.slice(1, tree.choices[j].elements.length));

                      // $+b | make a choice by $ & c <- added by @oruji
                      var _alt = makeAlt([rest1, rest2]);
                      // a($+b) | sequence $+b made with first common element <- added by @oruji
                      var _seq = makeSeq([first, _alt]);

                      // replace first choice "a" by created "a($+b)" <- added by @oruji
                      tree.choices[i] = _seq;
            ...
            // first choice is sequence -> abb*+a
              ...
              for (var j=i+1; j<tree.choices.length; j++) {
                if (tree.choices[j].tag !== tags.SEQ) {
                  ...
                  // if first Element of first SEQ choice "a" and second choice are equal <- added by @oruji
                  if (noam.util.areEquivalent(tree.choices[i].elements[0], tree.choices[j])) {
                    ...
                    if (noam.util.areEquivalent(first, last) && hasStar) {
                      // first common element "a" from second choice  <- added by @oruji
                      var first = tree.choices[j];
            
                      // select elements of first choice except first element <- added by @oruji
                      var rest1 = makeSeq(tree.choices[i].elements.slice(1, tree.choices[i].elements.length));
            
                      // second choice will be $ <- added by @oruji
                      var rest2 = makeEps();

                      // b+$ | make a choice by b & $ <- added by @oruji
                      var _alt = makeAlt([rest1, rest2]);
                      // a(b+$) | sequence b+$ made with last common element <- added by @oruji
                      var _seq = makeSeq([first, _alt]);

                      // replace first choice "ab" by created "a(b+$)" <- added by @oruji
                      tree.choices[i] = _seq;
